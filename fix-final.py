...
print('🔧 Criando arquivos do zero (100% limpos)...')

# 1. main.jsx - SEM capacitor
...
with open('src/main.jsx', 'w', encoding='utf-8') as f:
    f.write(main_code)
print('✅ main.jsx criado (sem capacitor)')

# 2. SetlistCard.jsx - 100% LIMPO
              ...
              {sortedSongs.map((item, idx) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-2 p-3 hover:bg-surface2 transition-colors">
                    ...
                    <button onClick={(e) => handleSongClick(song.id, e)} className="flex-1 min-w-0 text-left cursor-pointer hover:opacity-80 transition-opacity">
...
print('✅ SetlistCard.jsx criado')

# 3. Player.jsx - 100% LIMPO
...
with open('src/pages/Player.jsx', 'w', encoding='utf-8') as f:
    f.write(player_code)
print('✅ Player.jsx criado')

# 4. Home.jsx - 100% LIMPO
home_code = r"""import { useEffect, useState } from 'react'
import { Link } from 'react-router-dom'
import { supabase } from '../lib/supabase'
import { cacheSongs, getCachedSongs, cacheSetlists, getCachedSetlists, getLastSync } from '../services/cache'
...
export default function Home() {
  ...
  const fetchData = async () => {
    ...
    if (navigator.onLine) {
      try {
        const { data: songsData, error: songsError } = await supabase.from('songs').select('*').order('created_at', { ascending: false })
        if (songsError) throw songsError
        if (songsData) { setSongs(songsData); setFilteredSongs(songsData); await cacheSongs(songsData) }
        const { data: setlistsData, error: setlistsError } = await supabase.from('setlists').select('*, setlist_songs (id, position, songs (id, title, artist, original_key, bpm))').order('created_at', { ascending: false })
        if (setlistsError) throw setlistsError
        if (setlistsData) { setSetlists(setlistsData); await cacheSetlists(setlistsData) }
  ...
  const handleSearch = (query) => {
    ...
    const filtered = songs.filter(song => song.title.toLowerCase().includes(query.toLowerCase()) || song.artist.toLowerCase().includes(query.toLowerCase()))
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-bg" style={{ overflow: 'hidden' }}>
      <header className="flex-shrink-0 bg-black border-b border-gray-800 shadow-lg z-20" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        <div className="px-4 py-4">
          <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <img src="/icon2.png" alt="CifraBox" className="w-20 h-20 md:w-24 md:h-24 object-contain" />
              <h1 className="text-3xl md:text-4xl font-bold text-purple-400">CifraBox</h1>
            </div>
            <Link to="/editor" className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-4 py-2 rounded-xl transition-colors flex items-center gap-2 text-sm flex-shrink-0">
        ...
        <div className="px-4 pb-3 bg-black/40 backdrop-blur-lg">
          <div className="max-w-5xl mx-auto space-y-2">
            <div className="flex items-center gap-2">
              <span className={'text-xs px-2 py-1 rounded-full ' + (isOnline ? 'bg-green-600/20 text-green-400' : 'bg-orange-600/20 text-orange-400')}>
                {isOnline ? '🟢 Online' : '🔴 Offline'}
              </span>
              {lastSync && <span className="text-xs text-gray-300">Sync: {formatLastSync(lastSync)}</span>}
            </div>
            {!isOnline && (
              <div className="bg-orange-600/10 border border-orange-600/30 rounded-lg p-2 text-orange-400 text-xs flex items-center gap-2">
                <span>⚠️</span><span>Você está offline. Usando dados salvos.</span>
              </div>
            )}
            <div className="relative">
              <input type="text" value={searchQuery} onChange={e => handleSearch(e.target.value)} placeholder="🔍 Buscar músicas..." className="w-full bg-gray-900/90 border border-gray-700 rounded-xl px-4 py-2.5 pl-10 text-white focus:border-purple-500 outline-none text-sm" />
        ...
        <div className="px-4 border-t border-gray-700 bg-black/40 backdrop-blur-lg">
          <div className="max-w-5xl mx-auto flex gap-2">
            <button onClick={() => setActiveTab('songs')} className={'px-4 py-2.5 font-semibold text-sm transition-colors relative ' + (activeTab === 'songs' ? 'text-purple-400' : 'text-gray-400 hover:text-gray-300')}>
              🎵 Músicas ({songs.length})
              {activeTab === 'songs' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-purple-500"></div>}
            </button>
            <button onClick={() => setActiveTab('setlists')} className={'px-4 py-2.5 font-semibold text-sm transition-colors relative ' + (activeTab === 'setlists' ? 'text-purple-400' : 'text-gray-400 hover:text-gray-300')}>
        ...
        <div className="max-w-5xl mx-auto p-4 space-y-4">
          {activeTab === 'songs' ? (
            <div className="space-y-2">
              {filteredSongs.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <Link to="/editor" className="inline-block text-purple-400 hover:underline">Criar primeira cifra →</Link>
                </div>
              ) : (
            ...
            <div className="space-y-4">
              <button onClick={() => setShowCreateSetlist(true)} className="w-full bg-gradient-to-r from-purple-600/10 to-orange-500/10 border-2 border-dashed border-purple-500/40 hover:border-purple-500/70 rounded-xl p-4 text-purple-400 font-semibold transition-colors flex items-center justify-center gap-2">
                <span className="text-xl">+</span><span>Criar novo Setlist</span>
              </button>
              {setlists.length === 0 ? (
                <div className="text-center py-16">
                ...
                setlists.map(setlist => <SetlistCard key={setlist.id} setlist={setlist} onAdded={fetchData} onDeleted={fetchData} />)
              )}
            </div>
          )}
        </div>
      ...
      {showCreateSetlist && (
        <CreateSetlistModal onClose={() => setShowCreateSetlist(false)} onCreated={() => { setShowCreateSetlist(false); fetchData() }} />
      )}
    </div>
  )
}
...
with open('src/pages/Home.jsx', 'w', encoding='utf-8') as f:
    f.write(home_code)
print('✅ Home.jsx criado')

print('\n✅ TODOS OS ARQUIVOS FORAM RECRIADOS DO ZERO!')
print('\n🔨 AGORA execute EXATAMENTE estes comandos, um por um:')
print('   git add .')
print('   git commit -m "Corrige build - remove capacitor e erros de sintaxe"')
print('   git push --force')
print('\n⚠️  IMPORTANTE: O --force é OBRIGATÓRIO para sobrescrever o GitHub!')