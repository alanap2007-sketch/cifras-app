...
export default function Player() {
  ...
  const fetchSong = async () => {
    try {
      ...
      if (error) {
        ...
        } else { alert('Música não encontrada.'); navigate('/') }
      } else {
        setSong(data)
  ...
  const handleBack = () => {
    if (location.state?.from === 'setlist') { navigate(-1) } else { navigate('/') }
  }
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 space-y-4">
          ...
          <div className={`${surfaceColor} border ${borderColor} rounded-xl p-3 flex items-center justify-between flex-wrap gap-2`}>
            ...
            <div className="flex items-center gap-1.5 flex-wrap flex-shrink-0">
              ...
              {capo > 0 && (<span className="text-xs font-mono bg-accent2/10 text-accent2 px-2 py-0.5 rounded-full">Capo {capo}ª (efetivo: {effectiveKey})</span>)}
            </div>
          </div>
          <div className="space-y-3">
